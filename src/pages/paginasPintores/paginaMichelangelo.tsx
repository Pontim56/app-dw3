...
export default function PaginaMichelangelo() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch('/api/michelangelo/obraMichelangelo');
                if (!response.ok) {
                ...
                const response3 = await fetch('/api/michelangelo/getComentarios');
                if (!response3.ok) {
                    throw new Error('Erro ao buscar dados');
                }
                ...
                result.forEach(async (obra: Obra) => {
                    const responseMedia = await fetch(`/api/michelangelo/getMedia?id_obra=${obra.id_obra}`);
                    const resultMedia = await responseMedia.json();
                    setMedias((prevMedias) => ({
                        ...prevMedias,
                        [obra.id_obra]: resultMedia.media,
                    }));
                });
                setLoading(false);
            } catch (error: any) {
    ...
    const handleNotaSubmit = async (idObra: number) => {
        ...
        try {
            const response = await fetch('/api/michelangelo/addComentario', {
                method: 'POST',
                headers: {
    ...
    const handleDelete = async (id_comentario: number) => {
        try {
            const response = await fetch(`/api/michelangelo/deleteComentario?id_comentario=${id_comentario}`, {
                method: 'DELETE',
            });
                ...
                {data.map((item, index) => (
                    <div key={item.id_obra} className='flex items-center flex-col w-72  justify-between'>
                        ...
                        <div className='flex items-center flex-col'>
                            ...
                            {/* Exibir a média da nota */}
                            {/* <p>
                                Média:
                                {typeof medias[item.id_obra] === 'number'
                                    ? medias[item.id_obra].toFixed(2)
                                    : 'Sem notas'}
                            </p> */}
                        </div>
                        {/* <div className=' my-10'>
...
//     // Estados para o formulário de adicionar nova obra
...
//     // Função para enviar a nova obra
//     const handleSubmit = async (event: React.FormEvent) => {
//         event.preventDefault();
//         try {
//             console.log("Enviando dados:", { idObra, nomeObra, dataCriacao });
...
//             {/* Formulário de adicionar nova obra */}
//             <div className='my-10'>
//                 {!showForm && (
//                     <button
//                         className="p-2 bg-yellow-100 text-black rounded mt-4"
